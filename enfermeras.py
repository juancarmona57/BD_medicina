class Enfermera:   
    ...
    @classmethod
    def validar_cedula(cls, cedula):
        global visitas
        # Comprueba que la cédula tenga exactamente 10 dígitos y sea única.
        return len(cedula) == 10 and cedula not in [visita['cedula'] for visita in cls.visitas]

    @classmethod
    def registrar_datos_vitales(cls, cedula_paciente, presion, temperatura, pulso, oxigeno):
        global visitas

        if not cls.validar_cedula(cedula_paciente):
    ...
    def menu_enfermera(cls):
        ...
        while True:
            ...
            if opcion == "1":
                ...
                cls.registrar_datos_vitales(cedula, presion, temperatura, pulso, oxigeno)
            elif opcion == "2":
                cedula = input("Ingrese cédula del paciente: ")
